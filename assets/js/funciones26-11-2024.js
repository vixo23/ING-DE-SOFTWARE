        ...
        $.ajax({
            ...
            success: function (response) {
                response.forEach(row => {
                    // Generar el HTML con base en si el registro es editable o no
                    let editable = row.editable;  // 1 es editable, 0 no editable
                    ...
                    if (editable == 1) {  // Si es editable
                        html += `
                            <input type="number" class="form-control addCantidad mb-1" data-id="${row.id}" value="${row.cantidad}" min="1">
                            <button class="btn btn-danger btn-sm eliminarPlato" type="button" data-id="${row.id}">Eliminar</button>
                        `;
                    } else {  // Si no es editable
                        html += `
                            <input type="number" class="form-control addCantidad mb-1" data-id="${row.id}" value="${row.cantidad}" min="1" disabled>
                ...
                // Event listeners para los botones de eliminar y los inputs de cantidad

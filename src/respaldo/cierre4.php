            ...
            <div class="row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="monto_cierre" name="monto_cierre" 
                        value="<?php echo isset($monto_cierre) ? $monto_cierre : ''; ?>" 
                        placeholder="Ingrese monto de cierre" oninput="formatearCLP(this)" required>
                </div>
...
<script>
// Función para calcular el ajuste restando el monto de cierre de los ingresos
function calcularTotal() {
    var totalIngresos = <?php echo $totalMontoIngresos; ?>; // Ingresos totales (sin formato)
    var montoCierre = parseFloat(document.getElementById('monto_cierre').value.replace(/[^\d.-]/g, '')) || 0; // Convertir el valor de monto_cierre a número
    var ajuste = montoCierre - totalIngresos; // Calcular el ajuste
    document.getElementById('ajuste').value = ajuste.toLocaleString('es-CL', { minimumFractionDigits: 0, maximumFractionDigits: 0 }); // Mostrar el ajuste formateado
}
</script>

    ...
    <div class="formulario-control-caja">
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" id="formulario_cierre">
			...
			<div class="w-50 pl-2">
                    ...
                    <select class="form-control" id="id_cierres" name="id_cierres" required>
                        <option value="">Seleccione un cierre</option>
                        <?php
                        // Obtener todos los turnos con estado = 1
                        $query_cierres = "SELECT control_caja.*, cajas .descripcion as caja, usuarios.nombre as usuario, turnos.descripcion as turno
						FROM control_caja 
						...
						WHERE fecha_termino IS NOT NULL ORDER BY id_control_caja DESC";
                        $result_cierres = mysql_query($query_cierres, $conn);
                        if ($result_cierres) {
                            while ($rowc = mysql_fetch_assoc($result_cierres)) {
								$texto=$rowc['fecha_inicio']." | ".$rowc['usuario']." | ".$rowc['caja']." | ".$rowc['turno']." | ".$rowc['fecha_termino'];
                                echo "<option value='" . $rowc['id_control_caja'] . "'>" . $texto. "</option>";
                            }
                        } else {

...
if (isset($_GET['detalle'])) {
    ...
    if ($data) {
		...
		if ($_SESSION['rol']==1){
			$query_select = mysqli_query($conexion, "SELECT * FROM detalle_pedidos WHERE id = $id_detalle");
			$data = mysqli_fetch_assoc($query_select);
			...
			if ($data) {
				...
				// Verificar si la inserción en log fue exitosa
				if ($query_log) {
					// Eliminar el plato de detalle_pedidos
					$query_delete = mysqli_query($conexion, "DELETE FROM detalle_pedidos WHERE id = $id_detalle");
					if ($query_delete) {
						$msg = "ok";
					} else {
						$msg = "Error al eliminar el Producto";

...
if (isset($_GET['detalle'])) {
    ...
    $mesa = $_GET['mesa'];
		...
		//$sql_delete2="DELETE FROM temp_pedidos2 WHERE id_usuario='$id_usuario' AND id_sala='$id_sala' AND id_mesa='$mesa')";
		//$querydelete = mysqli_query($conexion, $sql_delete2);
    ...
    $sql = "
        SELECT dp.id, dp.cantidad, dp.precio, dp.id_producto, 
               ...
               p.nombre, p.precio AS precio_plato, '1' as impreso
        ...
        SELECT tp.id, tp.cantidad, tp.precio, tp.id_producto, 
               ...
               p.nombre, p.precio AS precio_plato, '0' as impreso
    ...
    ";
     ...
     if ($result > 0) {	
		 while ($data_temp2 = mysqli_fetch_assoc($query_temp2)) { 
			...
			$idx= $data_temp2['id'];
			$impreso= $data_temp2['impreso'];
				
			$sql_temp2="INSERT INTO temp_pedidos2(cantidad, precio, id_producto, id_usuario, id_sala, id_mesa, id_pedido, impreso) VALUES ($cantidad, $preciox, $id_producto, $id_usuario, $id_sala, $mesa, $idx, $impreso )";
			$queryinsert_temp2 = mysqli_query($conexion, $sql_temp2);
		 }
	 }
    ...
    if ($pedidoPendiente) {
		...
		$sqlw = "SELECT d.*, e.observacion,i.id_impresora,  p.nombre, p.precio, e.id_garzon FROM detalle_pedidos d 
       ...
       //	$consulta = mysqli_query($conexion, "SELECT d.*, p.nombre FROM temp_pedidos2 d INNER JOIN platos p ON d.id_producto = p.id WHERE d.id_usuario = $id_user AND d.id_sala=$id_sala AND d.id_mesa=$mesa");
        $consulta = mysqli_query($conexion,$sqlw);
		if (!$consulta) {
            echo json_encode(array('mensaje' => 'error', 'error' => 'Error al obtener los detalles del pedido temporal.'));
            die();
        }
        ...
        while ($row = mysqli_fetch_assoc($consulta)) {
			...
			if ($impreso==0){
			//$sql_detalle = "INSERT INTO detalle_pedido (id_pedido, id_empresa, id_impresora, codigo_producto, descripcion, cantidad, observacion, precio, estado, salon, mesa, garzon) 
			//VALUES ($id_pedido, $id_empresa, $id_impresora, $id_producto, '$nombre', $cantidad, '$observacion', $precio, 0, '$sala', $mesa, '$garzon')";
...
if (isset($_POST['regDetalle'])) {
    ...
    if (empty($row)) {
		...
		$query2 = mysqli_query($conexion, "INSERT INTO temp_pedidos2 (cantidad, precio, id_producto, id_usuario, id_sala, id_mesa, impreso) VALUES (1, $precio, $id_producto, $id_user, $id_sala, $mesa, '0')");
    } else {

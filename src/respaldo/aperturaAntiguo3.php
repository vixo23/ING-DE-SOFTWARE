...
$id_caja = '';
$id_turno = '';
$fecha_inicio = date('Y-m-d'); // Establecer la fecha de inicio como la fecha actual
$monto_apertura = '';
$alert = ''; // Inicializar variable de alerta
...
if (mysql_num_rows($result_verificar_caja) > 0) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query_check = "SELECT * FROM control_caja WHERE id_caja = '$id_caja' AND id_turno = '$id_turno' AND fecha_inicio = '$fecha_inicio'";
        ...
        <select class="form-control" id="id_caja" name="id_caja" required>
            ...
            if ($result_cajas) {
                while ($row = mysql_fetch_assoc($result_cajas)) {
                    echo "<option value='" . $row['id_caja'] . "' " . ($row['id_caja'] == $id_caja ? 'selected' : '') . ">" . htmlspecialchars($row['descripcion']) . "</option>";
                }
            } else {
        ...
        <select class="form-control" id="id_turno" name="id_turno" required>
            ...
            if ($result_turnos) {
                while ($row = mysql_fetch_assoc($result_turnos)) {
                    echo "<option value='" . $row['id_turno'] . "' " . ($row['id_turno'] == $id_turno ? 'selected' : '') . ">" . htmlspecialchars($row['descripcion']) . "</option>";
                }
            } else {
            ...
            <div class="form-group d-flex justify-content-between">
                <div class="w-50 pr-2">
                    ...
                    <input type="date" class="form-control" id="fecha_inicio" name="fecha_inicio" value="<?php echo $fecha_inicio; ?>" required readonly>
                </div>
                <div class="w-50 pl-2">

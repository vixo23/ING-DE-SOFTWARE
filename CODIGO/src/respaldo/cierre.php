...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['monto_cierre']) && isset($_POST['id_control_caja'])) {
    $monto_cierre = $conexion->real_escape_string($_POST['monto_cierre']);
    $ajuste = isset($_POST['ajuste']) ? $conexion->real_escape_string($_POST['ajuste']) : 0;
    $id_control_caja = $conexion->real_escape_string($_POST['id_control_caja']);
    $fecha_termino = isset($_POST['fecha_termino']) ? "'" . $conexion->real_escape_string($_POST['fecha_termino']) . "'" : "NULL";

    $queryUpdate = "UPDATE control_caja SET monto_cierre = '$monto_cierre', ajuste = '$ajuste', fecha_termino = $fecha_termino WHERE id_control_caja = '$id_control_caja'";
...
if ($query->fetch()) {
    ...
    $montoApertura = $montoApertura ? $montoApertura : 0;
    $queryMontoApertura->close();

    $sumaTotalIngresos = $totalMontoIngresos + $montoApertura;
    foreach ($montosPagos as $monto) {
        $sumaTotalIngresos += $monto;
    }
}

// Consulta adicional de ingresos
...
// Consulta final para obtener fecha de inicio
$queryFechaInicio = $conexion->prepare("SELECT fecha_inicio FROM control_caja WHERE id_control_caja = ?");
$queryFechaInicio->bind_param("i", $id_control_caja);
$queryFechaInicio->execute();
$queryFechaInicio->bind_result($fecha_inicio);
$queryFechaInicio->fetch();
$queryFechaInicio->close();
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" autocomplete="off" id="formulario">
                <div class="row">
                    ...
                    <div class="form-group">
                        <label for="fecha_inicio">Fecha Hora Apertura</label>
                        <?php
    ...
    <div class="row mt-3">
        <div class="col-md-9">
            ...
            <select class="form-control" id="id_cierres" name="id_cierres">
                ...
                $result_cierres = mysqli_query($conn, $query_cierres);

                if ($result_cierres) {
                    while ($rowc = mysqli_fetch_assoc($result_cierres)) {
                        ...
                        echo "<option value='" . htmlspecialchars($rowc['id_control_caja']) . "' 
                                data-fecha-inicio='" . htmlspecialchars($rowc['fecha_inicio']) . "' 
                                data-usuario='" . htmlspecialchars($rowc['usuario']) . "' 
                                data-caja='" . htmlspecialchars($rowc['caja']) . "' 
                                data-turno='" . htmlspecialchars($rowc['turno']) . "' 
                                data-monto-apertura='" . htmlspecialchars($rowc['monto_apertura']) . "' 
                                data-monto-cierre='" . htmlspecialchars($rowc['monto_cierre']) . "' 
                                data-ajuste='" . htmlspecialchars($rowc['ajuste']) . "'>" . 
...
function redirectToCierreHistorico() {
    ...
    if (selectedValue) {
        var url = 'cierre_Historico.php?id_cierre=' + encodeURIComponent(selectedValue);

...
if (isset($_GET['id'])) {
    ...
    $query = mysqli_query($conexion, "SELECT status FROM tipo_vacaciones WHERE id_tipovacacion = '$id'");
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $update = mysqli_query($conexion, "UPDATE tipo_vacaciones SET status = '$nuevo_estado' WHERE id_tipovacacion = '$id'");

        // Verificar si la actualización fue exitosa
        if ($update) {
            header("Location: tipo_vacaciones.php?mensaje=Estado cambiado con éxito");
        } else {
            header("Location: tipo_vacaciones.php?error=Error al cambiar el estado");
        }
    } else {
        // Si no se encuentra el garzón, redirigir con un mensaje de error
        header("Location: tu_pagina.php?error=tipo vacacion no encontrado");
    }
} else {

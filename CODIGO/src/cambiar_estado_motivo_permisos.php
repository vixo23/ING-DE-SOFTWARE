...
if (isset($_GET['id'])) {
    ...
    $query = mysqli_query($conexion, "SELECT status FROM motivos_permisos WHERE id_motivo = '$id'");
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $update = mysqli_query($conexion, "UPDATE motivos_permisos SET status = '$nuevo_estado' WHERE id_motivo = '$id'");

        // Verificar si la actualización fue exitosa
        if ($update) {
            header("Location: motivo_permisos.php?mensaje=Estado cambiado con éxito");
        } else {
            header("Location: motivo_permisos.php?error=Error al cambiar el estado");
        }
    } else {
